			...
			if ( is_wp_error( $post_tag_obj ) ) {
				...
				if(empty($user_checked_tag)){
					...
					if(array_key_exists($post_id, $user_checked_tag))
					{
						...
						$recursive = $user_checked_tag + array($post_id => array($post_tag_obj));
						update_user_meta($user_id, 'startup_tag_ids', $recursive);
					}
				}
			}
			wp_send_json_success(array('post_id'=>$post_tag_obj, 'message'=>'New StarUp Tag Added.'));
		}
    }else{
 ...
 * WatchList List Page
 */
add_shortcode('watch_list_page', function(){
	ob_start();
	if ( is_user_logged_in() ) {    
		?>
