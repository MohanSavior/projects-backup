...
jQuery(document).ready(function ($) {
	...
	hartIcon.length > 0 ? jQuery('.add-post-tag-cls i').css('color', '#a71c44') : '';
...
if (typeof watchlistsCheckboxes !== 'undefined') {
	...
	const startupAddRemove = (watchlistsCheckboxes) => {
		...
		CboxesVal.length > 0 ? jQuery('.add-post-tag-cls i').css('color', '#a71c44') : jQuery('.add-post-tag-cls i').css('color', '');
		delay(() => {
...
/**
 * Pre Render Current User First Name & Last Name
 */
jQuery('.offers-revice-cls, #make_offer_form_click').on('click', function(e) {
	setTimeout(() => {
		jQuery('#input_3_1_3, #input_7_1_3').attr("value", savior_ajax_obj.current.first_name);
		jQuery('#input_3_1_6, #input_7_1_6').attr("value", savior_ajax_obj.current.last_name);
		jQuery('#input_3_3, #input_7_3').attr("value", savior_ajax_obj.current.phone_number);
		jQuery('#input_3_7, #input_7_7').attr("value", savior_ajax_obj.current.organization);
		jQuery('#input_7_18').attr("value", savior_ajax_obj.current.offer_value);
	}, 500);
})
// jQuery(document).on('gform_post_render', function (event, form_id, current_page) {
// 	if (form_id == 3 || form_id == 7) {
// 	}
// });
/**
 * Make an offer btn hide show
 */
if (savior_ajax_obj.check_offer_post == 1) {
	jQuery('#make_offer_form_click').remove();
}
jQuery(document).on('gform_confirmation_loaded', function (event, formId) {
	if (formId == 3) jQuery('#make_offer_form_click').remove();
});
/**
 * Cancel Offer
 */
jQuery('#cancel_offer').on('click', function ($) {
