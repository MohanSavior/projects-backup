...
class Companies{
    ...
    public function prepare_company_information( $form )
    {
        ...
        if($this->is_custom_post_id($post_id))
        {
            ...
            foreach( $form['fields'] as &$field )  {
                ...
                if ( $field->id == 35 ) {
                    $field->defaultValue = date("m-d-Y", strtotime(get_post_meta($post_id, 'date_approved_on_exhibitors_list', true)));
                }
                //Company Address:
                if ( $field->id == 4 ) {
                ...
                if ( $field->id == 15 ) {
                    $field->defaultValue = get_user_meta(get_post_meta($post_id, 'primary_booth_admin', true), 'billing_phone', true);
                }
                ...
                if ( $field->id == 40 ) {
                    $field->defaultValue = $alternate_booth_admin->user_email;
                }
    ...
    public function update_company_information( $entry, $form )
    {
      ...
      if(!$primary_booth_admin_id = email_exists( $_POST['input_13'] ))
      {
        ...
        update_user_meta($primary_booth_admin_id, 'last_name', $entry['10.6']);
        update_user_meta($primary_booth_admin_id, 'billing_phone', $entry[15]);
        error_log(print_r('New primary_booth_admin_id created successfully', true));
        error_log(print_r($primary_booth_admin_id, true));
      }
      if(!$alternate_booth_admin_id = email_exists( $_POST['input_40']))
      {
        ...
        error_log(print_r('New alternate_booth_admin_id created successfully', true));
        error_log(print_r($alternate_booth_admin_id, true));
      }
      ...
      if (is_wp_error($result)) {
          ...
          error_log(print_r('Post updated successfully', true));
