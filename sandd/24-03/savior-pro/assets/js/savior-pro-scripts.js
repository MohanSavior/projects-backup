...
if (typeof watchListsPostWithTag !== 'undefined') {
	watchListsPostWithTag.forEach(item => {
		item.addEventListener('change', (event) => {
			startUpPostRemove(event.target.value, event.target.getAttribute('data-tag_id'));
		});
	})
	...
	Array.from(document.querySelectorAll('.watchlists-post-with-tag-li')).forEach(item => {
		item.addEventListener('mouseover',  (event) => {
			...
			let postEl  = document.getElementById(`watchlist-post-tag-${inputEl.value}`);
    ...
    // Array.prototype.forEach.call(test, item => {
	// });
}