...
class ExhibitorManagement {
    public function __construct() {
      ...
      if(is_admin())
      {
        ...
        add_action( 'wp_ajax_company_delete', array($this, 'company_delete_callback'));
      }
                              ...
                              if (!empty($booth_admin_ids)) {
                                  foreach ($booth_admin_ids as $customer_id) {
                                      ...
                                      if (!empty($last_order_id)) {
                                          $order_status[] = $last_order_id['order_status'];
                                      }
                                  }
                              }
      ...
      if(is_admin())
      {
        ...
        if(!empty($post_ids))
        {
          ...
          foreach ($post_ids as $post_id) {
            ...
            }elseif($alternate_booth_admin_id){
            ...
            //exhibitor-members-list
            $billing_phone = get_user_meta($primary_booth_admin_id, 'billing_phone', true);
            $primary_booth_admin_contact = get_user_meta($primary_booth_admin_id, 'primary_booth_admin_contact', true);
            $user_phone = get_user_meta($primary_booth_admin_id, 'user_phone', true);
            $primary_contact = !empty($billing_phone) ? $billing_phone : ($primary_booth_admin_contact ? $primary_booth_admin_contact : $user_phone);
            $cleanedNumber = str_replace(['(', ')', ' ', '-'], '', $primary_contact);
            $formattedNumber = substr_replace($cleanedNumber, '-', 3, 0);
            $primary_contact = substr_replace($formattedNumber, '-', 7, 0);

            if($alternate_booth_admin_id){
              $alternate_booth_phone  = get_user_meta($alternate_booth_admin_id, 'billing_phone', true);
              $alternate_booth_phone  = $alternate_booth_phone ? $alternate_booth_phone : get_user_meta( $alternate_booth_admin_id, 'user_phone', true );
              
              $alternate_cleanedNumber = str_replace(['(', ')', ' ', '-'], '', $alternate_booth_phone);
              $alternate_formattedNumber = substr_replace($alternate_cleanedNumber, '-', 3, 0);
              $alternate_booth_phone = substr_replace($alternate_formattedNumber, '-', 7, 0);
            }else{
              $alternate_booth_phone='';
            }

            $company_address = array(
              get_post_meta($post_id, 'company_address', true),
              get_post_meta($post_id, 'company_city', true),
              get_post_meta($post_id, 'company_state', true),
              get_post_meta($post_id, 'company_postcode', true),
              get_post_meta($post_id, 'company_country', true)
            );
            $company_primary_order = $this->get_last_order_id_by_product_and_customer(18792, $primary_booth_admin_id );
            if( $company_primary_order && isset($company_primary_order['order_status']) && $company_primary_order['order_status'] == 'completed')
            {
              $company_order = $company_primary_order;
            }elseif($alternate_booth_admin_id){
              $company_order = $this->get_last_order_id_by_product_and_customer(18792, $alternate_booth_admin_id );
            }else{
              $company_order = $company_primary_order;
            }
            error_log(print_r($company_order,true));
            $companies_data[] = array(
                'no'                    => '',
                'action'                => '',
                ...
                'order_created_date'    => $company_order['order_created_date'],
                'order_paid_date'       => $company_order['order_paid_date'],
                'order_status'          => $company_order['order_status'] == 'completed' ? "<span style='color:green'>".ucfirst($company_order['order_status'])."</span>" : "<span style='color:red'>".ucfirst($company_order['order_status'])."</span>" ,
                ...
                'company_address'       => implode(',', array_filter($company_address)),
                ...
                'alternate_contact'     => $alternate_booth_phone ? $alternate_booth_phone : '-',
    ...
    public function create_order( $product_id, $customer_id, $qty = 0, $company_id, $user_employer = null, $relation_between_orders )
    {
      try {
        WC()->cart->empty_cart(); 
    ...
    public function get_booth_totalizer() {
      ...
      echo (176 - $total_qty) .' of 176 Booths remains';
    }
    ...
    public function get_last_order_id_by_product_and_customer($product_id, $customer_id) 
    {
      ...
      if(!empty($last_order_id))
      {
        ...
        $order_arr['order_status']        = $order->get_status();
        $order_arr['order_created_date']  = $order->get_date_created()->date('Y-m-d');
        $order_arr['order_paid_date']     = $order->get_status() == 'completed' ? $order->get_date_paid()->date('Y-m-d') : '-';
      }
      $order_arr = wp_parse_args($order_arr, $defaults);
      return $order_arr;
    }

    public function company_delete_callback()
    {
      if(isset($_POST['company_id']) && !empty($_POST['company_id']))
      {
        $company_update = wp_update_post(array('ID' => $_POST['company_id'], 'post_status'   => 'trash'));
        if(is_wp_error( $company_update ))
        {
          wp_send_json_success();
        }else{
          wp_send_json_success();
