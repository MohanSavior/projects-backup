...
{
	$classes = array();
	if(isset($_COOKIE['darkModeEnabled']) && $_COOKIE['darkModeEnabled'] == 'true' && !isset($_COOKIE['bbtheme']) ) {
		...
		if( get_post_meta(get_the_ID(), 'select_theme', true) == 'dark' ){
			$classes[] = 'dark-mode-active';
		}elseif ( (in_array('buddypress', get_body_class())) && (!isset($_COOKIE['bbtheme']) || empty($_COOKIE['bbtheme']) ) ) {
			$classes[] = 'dark-mode-active';
		}else{
			$classes[] = 'bbDark1';
		}
	}elseif ( (in_array('buddypress', get_body_class()) || in_array('single-sfwd-courses', get_body_class())) && !isset($_COOKIE['bbtheme']) ) {
		$classes[] = 'dark-mode-active';
	}elseif ( (in_array('buddypress', get_body_class()) || in_array('single-sfwd-courses', get_body_class())) && (isset($_COOKIE['bbtheme']) && $_COOKIE['bbtheme']  == 'dark' )) {
			$classes[] = 'dark-mode-active';
	}else{
		$classes[] = 'bbDark2';
	}
...
add_filter( 'body_class', function( $classes ) {
	...
	elseif ( (in_array('single-sfwd-topic', $classes) || in_array('single-sfwd-lessons', $classes) || in_array('single-sfwd-courses', $classes)) && ( (isset($_COOKIE['bbtheme']) && $_COOKIE['bbtheme'] == 'dark' ) || !isset($_COOKIE['bbtheme'])) ) {
        return array_merge( $classes, array( 'bb-dark-theme' ) );
    }
