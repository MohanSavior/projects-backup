...
add_action( 'gform_advancedpostcreation_post_after_creation_6', 'advancedpostcreation_checkboxes', 10, 4 );
	function advancedpostcreation_checkboxes( $post_id, $feed, $entry, $form ) {
		...
		if(!empty($entry['17'])){
			...
			$image = rudr_upload_file_by_url($image_url);
			update_post_meta($post_id, 'main_photo', $image);
			update_post_meta($post_id, '_main_photo', 'field_637c9a7916892');
		}
		if(!empty($entry['19'])){
			$image_url = trim(str_replace( array( '\'', '"', ',' , ';', '<', '>', '[', ']' ), ' ', $entry['19']));
			$image = rudr_upload_file_by_url($image_url);
			update_post_meta($post_id, 'additional_photo_1', $image);
			update_post_meta($post_id, '_additional_photo_1', 'field_637c9a9e16895');
		}
		if(!empty($entry['21'])){
			$image_url = trim(str_replace( array( '\'', '"', ',' , ';', '<', '>', '[', ']' ), ' ', $entry['21']));
			$image = rudr_upload_file_by_url($image_url);
			update_post_meta($post_id, 'additional_photo_2', $image);
			update_post_meta($post_id, '_additional_photo_2', 'field_637c9ad016896');
		}
		if( isset($_POST['gform_uploaded_files']) )
		{
			foreach(json_decode(stripslashes($_POST['gform_uploaded_files'])) as $image_id => $images )
			{
...
add_shortcode( 'test', function(){
	echo"<pre>";
		...
		print_r(get_post_meta(2081));echo"<br>";
