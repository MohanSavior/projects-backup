...
function get_name_business_profile()
{
	...
	{ 
		...
		$author_id = (int)get_post($business_profile_post_id)->post_author;

		if( !in_array( get_current_user_id(), array($author_id,(int)get_post()->post_author)) )
		{
			return "<p>You are not a <b>Buyer or Seller</b> of this post.";
		}else{
			?>
			<div class="offer-single-main-cls">
				...
				<div class="offer-post-header-btn">
					...
					<div class="right-side-btn">
						<?php 
							if(get_current_user_id() !== (int)get_post($business_profile_post_id)->post_author){
								echo '<a href="mailto:'.get_the_author_meta('user_email', $author_id).'">Contact Seller</a>';
							}else{
								echo '<a href="mailto:'.get_the_author_meta('user_email', (int)get_post()->post_author).'">Contact Buyer</a>';
							}
							$count_comment = wp_count_comments(get_the_ID());
							if(get_current_user_id() === (int)get_post($business_profile_post_id)->post_author && $count_comment->approved == 0){
							...
							if(get_current_user_id() === (int)get_post(get_the_ID())->post_author){
								echo '<a href="javascript:void(0)" data-id="'.get_the_ID().'" id="cancel_offer">Cancel Offer <i class="fas fa-spinner fa-spin"></i></a>';
							}
						?>
			...
			<?php 
			if(comments_open() && $count_comment->approved > 0)
				comments_template(); 		
		}
	} 
	$output = ob_get_contents();
...
add_filter( 'get_comment_text', function( $comment_comment_content, $comment, $args ){
    ...
    // if( doing_action( 'wp_ajax_get-comments' ) ){
        ...
        $counter_offer = get_comment_meta( $comment->comment_ID, 'counter_offer', true );

        if($counter_offer)
            echo  __('Offer Value') . ': ' . $counter_offer;
...
add_filter( 'get_comment_author', function ($author, $comment_id, $comment) {
	if($comment->user_id == (int)get_post($comment->comment_post_ID)->post_author )
	{
		return $author . ' ( Buyer )';
	}else{
...
function save_comment_meta_data( $comment_id ) {
    add_comment_meta( $comment_id, 'counter_offer', $_POST[ 'counter_offer' ] );
}
function add_my_custom_field_to_comment_form() {
    ?>
    	<p class="comment-form-counter_offer">
            <label for="counter_offer"><?php _e( 'Offer Value', 'savior-pro' ); ?><span class="required">*</span>:</label>
            <input id="counter_offer" name="counter_offer" value="" placeholder="" />
        </p>
    <?php
}
...
add_filter('comment_form_defaults', function($arg) {
	$arg['label_submit'] = 'Submit Offer Message';
	return $arg;
});
