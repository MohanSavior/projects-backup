...
jQuery(document).ready(function ($) {
        ...
        buttons: [
            ...
            },
            {
                ...
                action: function (e, dt, node, config) {
                    ...
                    let day_pass = {};
                    let rowCount = 0;
                    dt.rows('[data-printed="0"]').every(function (rowIdx) {
                        if (rowCount < 25) 
                        {
                            console.log(rowIdx);
                            var rowData = this.data();
                            let flagColumns = [8, 9, 10, 11, 12];
                            let flags = {};
                            $.each(flagColumns, function (index, columnIndex) {
                                var flagName = getColumnFlagName(columnIndex);
                                flags[flagName] = $(rowData[columnIndex]).text();
                            });
                            customer_flags[`user_flags_${rowData[rowData.length - 1]}`] = flags;
                            customer_ids.push(rowData[rowData.length - 1]);
                            day_pass[`user_day_pass_${rowData[rowData.length - 1]}`] = $(rowData[2]).data('product_id') == 18784 ? true : false;
                            rowCount++; // Increment the counter
                        } else {
                            return false; // Exit the loop after 25 rows
                        }
                    });
    ...
    $(document).on('click', 'button[id*="member_print_"]', function (e) {
        ...
        $.each(flagColumns, function (index, columnIndex) {
            var role = getColumnFlagName(columnIndex);
            flags[`${role}`] = $(rowData[columnIndex]).text();
        });
        day_pass[`user_day_pass_${customer_ids}`] = $(this).data('product_id') == 18784 ? true : false;
        customer_flag[`user_flags_${customer_ids}`] = flags;
