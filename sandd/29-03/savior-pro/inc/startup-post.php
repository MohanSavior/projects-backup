...
function startup_post_template( $atts )
{
    ...
    ?>
        <div class="startup-post-list-cls startup-post-<?=get_the_ID()?>" id="watchlist-post-<?=get_the_ID()?>-tag-<?=$attr['tag_id']?>" data-mrr="<?=get_field( "mrr", $attr['post_id'] )?>" data-cltv="<?=get_field( "cltv", $attr['post_id'] )?>" data-cac="<?=get_field( "cac", $attr['post_id'] )?>">                           
            <?php
