...
function mobile_text_alerts_on_mobile_blur() {
	...
	$body_param['first_name'] 	= !empty( $_POST['billing_first_name'] ) ? $_POST['billing_first_name'] : '';
	$body_param['last_name'] 	= !empty( $_POST['billing_last_name'] ) ? $_POST['billing_last_name'] : '';
	$body_param['email'] 		= !empty( $_POST['billing_email'] ) ? $_POST['billing_email'] : '';

	if( $body_param['number'] && $body_param['first_name'] && $body_param['last_name'] && $body_param['email'] ){
		$response = wp_remote_post( $url, array(
			'method'      => 'POST',
			'blocking'    => true,
			'headers'     => array(),
...
function option_fundamentals_members_area_sections_basic( $sections ) {
	//$sections['tradealerts'] = __( 'Trade Alerts', 'option-fundamentals' );
	$sections = array('trade-alerts' => __( 'Trade Alerts Basic', 'option-fundamentals' )) + $sections;
	return $sections;
}
...
function premium_user_urls(){
	...
	foreach ($memberships_info as $membership_id => $membership) {
		$membership_id = array($membership->plan->get_id());
		// 		print_r($membership_id);
		if(is_user_logged_in() && in_array('48142',$membership_id)){
...
function standard_user_urls(){
	...
	foreach ($memberships_info as $membership_id => $membership) {
		$membership_id = array($membership->plan->get_id());
		// 		print_r($membership_id);
		if(is_user_logged_in() && in_array('9506',$membership_id)){
			$url = '<a href="'.site_url().'/my-account/members-area/9506/my-membership-content/">Standard Members</a>';
		}
		else{
