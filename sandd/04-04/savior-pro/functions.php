						...
						<div class="post-detail-table" style="overflow-x:auto;">
							<a href="<?php echo get_permalink( get_the_ID() ); ?>">
								<table>
									...
									<tr>
										<td>Offer Value</td>
										<?php printf('<td>$%s</td>', number_format(get_post_meta( get_the_ID(), 'offer_value', true ),0,".",",")); ?>
									</tr>
									<tr>
										<td>Sent Date</td>
		...
		<div class="offer-single-main-cls">
			...
			<div class="offer-post-header-btn">
				...
				<div class="right-side-btn">
					...
					<a href="javascript:void(0)" class="offers-revice-cls">Revice Offer</a>
...
function comments_on_offers( $data ) {
    if( $data['post_type'] == 'offera' ) {
        $data['comment_status'] = 1;
    }
    return $data;
}
add_filter( 'wp_insert_post_data', 'comments_on_offers' );
...
function after_submission_counter_offers(  $entry, $form  ) { 
	if(is_singular( 'offers' )){
		...
		error_log(print_r(rgar( $entry, '14' ),true));
		error_log(print_r($offer_comment_field,true));
