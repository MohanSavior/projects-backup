...
class Print_Badges
{
 ...
 ON DUPLICATE KEY UPDATE id=VALUES(id),printed_count=VALUES(printed_count);");
    ...
    public function print_badge_qr_code_fn( $atts )
    {
        ...
        $user = get_userdata($atts['current_user_id']);
        if(is_user_logged_in())
        {
            $current_user               = $user;
            $user_obj                   = array();
            $user_obj['name']           = $current_user->user_login;
            ...
            $user_obj['role']           = $current_user->roles;
            
            $user_obj['user_title']     = get_user_meta( $current_user->ID, 'user_title', true );
            $user_obj['user_employer']  = get_user_meta( $current_user->ID, 'user_employer', true );
            ...
            $user_obj['phone_daytime']  = get_user_meta( $current_user->ID, 'billing_phone', true );
            $user_obj['addr_addr_1']    = get_user_meta( $current_user->ID, 'billing_address_1', true );
            $user_obj['addr_addr_2']    = get_user_meta( $current_user->ID, 'billing_address_2', true );
            $user_obj['addr_city']      = get_user_meta( $current_user->ID, 'billing_city', true );
            $user_obj['addr_state']     = get_user_meta( $current_user->ID, 'billing_state', true );
            $user_obj['addr_country']   = get_user_meta( $current_user->ID, 'billing_country', true );
            $user_obj['addr_zip']       = get_user_meta( $current_user->ID, 'billing_postcode', true );
            printf('
                ...
                preg_replace('!\s+!', ' ', $user_obj['addr_addr_1']. ' ' .$user_obj['addr_addr_2']. ' ' .$user_obj['addr_city']. ' ' .$user_obj['addr_state']. ' ' .$user_obj['addr_country']. ' ' .$user_obj['addr_zip']),
