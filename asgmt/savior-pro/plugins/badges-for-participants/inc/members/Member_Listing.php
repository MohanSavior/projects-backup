...
class Member_Listing
{
            ...
            <div class="member-list" >      
                ...
                <table id='member-listing' class='display nowrap'>
                    ...
                    <?php
                        ...
                        //     {
                        //         if (!in_array( $members_list->product_id, array(27011, 27010, 18792) )) {
                        //             //continue;
                        //             $user_id = $members_list->customer_id;
                        //             $member_user = new WP_User( $user_id );
                        //             $instructor_selected = $this->user_has_role($user_id, 'speaker') ? 'checked="checked"' : '';
                        //             $speaker = '<div class="button r" id="button-1"><input type="checkbox" onchange="instructor_member(this,'.$user_id.')" value="0" '.$instructor_selected.' class="checkbox" /><div class="knobs"></div><div class="layer"></div></div>';
                        //             ++$key;
                        ...
                        //                     <td>%s</td>
                        //                     <td class='user_badge_print_%s'>%s</td>
                        //                     <td>
                        //                         <button type='button'data-customer_id='%s' data-order_id='%s' data-product_id='%s' id='member_print_%s'>%s</button>
                        //                         </td>
                        //                     <td>%s</td>
                        //                     <td>%s</td>
                        //                     <td>%s</td>
                        ...
                        //                 ",
                        //                 $key.'_'.$user_id,
                        //                 !empty($get_user_ids_printed) && in_array($user_id, $get_user_ids_printed) ? '1' : '0',
                        //                 $key,
                        //                 $user_id,
                        //                 !empty($get_user_ids_printed) && in_array($user_id, $get_user_ids_printed) ? '<b style="color:green;">Printed</b>' : '<b style="color:red;">Not printed</b>',
                        //                 $user_id,
                        //                 $members_list->order_id,
                        //                 $members_list->product_id,
                        //                 $members_list->order_id.'_'.$user_id,
                        //                 !empty($get_user_ids_printed) && in_array($user_id, $get_user_ids_printed) ? 'Printed' : 'Print',
                        //                 $members_list->first_name,
                        //                 $members_list->last_name,
                        //                 $members_list->customer_email,
                        //                 $members_list->company,
                        //                 $members_list->product_name,
                        //                 in_array('BM', $role_flag) ? '<b style="color:green;">Y</b>' : '<b style="color:red;">N</b>',
                        //                 in_array('CM', $role_flag) ? '<b style="color:green;">Y</b>' : '<b style="color:red;">N</b>',
                        //                 in_array('SP', $role_flag) ?'<b style="color:green;">Y</b>' : '<b style="color:red;">N</b>',//$speaker,
                        //                 in_array('EX', $role_flag) ? '<b style="color:green;">Y</b>' : '<b style="color:red;">N</b>',
                        //                 in_array($members_list->product_id, $ceu_product_ids) ? '<b style="color:green;">Y</b>' : '<b style="color:red;">N</b>',
                        //                 $user_id,
                        //             );
                        //         }
                        //         unset($role_flag); 
    ...
    public function print_badges( $atts )
    {
        ...
        if(empty($user_ids))
        {
            wp_send_json_error(array('message' => 'The badges that are not printed are not found.'));
        }
            ...
            <div class="badge_1 <?php echo 'tpl-' . $format_type; ?> daypass <?php echo $format_type == 2 ? 'ready' : ''; ?>"
                        ...
                        <div class="left">
                            <div class="flex-block">
                                <div class="top">
                                    ...
                                    <div class="user-job">
                                        <span class="full-width">
                                            <?php echo $this->word_font_size_reduce( $rec['job'], 20 ); ?>
                                        </span>
                                    </div>
                                </div>
                                <div class="user-company-address">
                                    <div class="user-company">
                                        <span class="full-width"><?php echo $this->word_font_size_reduce( $rec['company'], 18 ); ?></span>
                                    </div>
                ...
                <?php else: ?>
                            ...
                            <div class="left">
                                ...
                                <div class="flex-block">
                                    <div class="top">
                                        ...
                                        <?php $ps = strlen($rec['job']) > 24 ? 12 : 14; ?>
                                        <div class="user-job">
                                            <span class="full-width"><?php echo $this->word_font_size_reduce( $rec['job'], 18 ); ?></span>
                                        </div>
                                    </div>
                                    <div class="user-company-address">
                                        <div class="user-company">
                                        ...
                                        <?php $lsize = strlen($user->user_login) > 24 ? 10 : 14; ?>
                                        <!-- <div class="user-email" style="font-size: <?//=$lsize?>px !important;"><?php //echo $user->user_login;?></div> -->
                                        <!-- <div class="user-contact" style="font-size: 12px !important;"><?php //echo $primary_contact;?></div> -->
                                        <div class="user-address">
                                            <span class="full-width">
                                                <?php
                        ...
                        <div class="daypass-footer">
                            <?php 
                                $day_pass = isset($_POST['day_pass']) ? $_POST['day_pass'] : false;
    ...
    public function reset_print_status_callback()
    {
        ...
        $update_reset_print = update_option( 'badge_print_ids',array());
        if(is_wp_error( $update_reset_print ))
        {
    ...
    public function get_member_list_callback()
    {
        ...
        if (!empty($search_term)) {
            $where_condition .= " AND (`first_name` LIKE '%$search_term%' OR `last_name` LIKE '%$search_term%' OR `customer_email` LIKE '%$search_term%' OR `company` LIKE '%$search_term%' OR `product_name` LIKE '%$search_term%') AND product_id NOT IN (27011, 27010, 18792)";
        }

        // Build the query based on filters and search
        $query = "SELECT * FROM $attendee_badge_orders WHERE $where_condition AND product_id NOT IN (27011, 27010, 18792) LIMIT $start, $length";
