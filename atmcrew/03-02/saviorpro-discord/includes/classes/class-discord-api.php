...
class SaviorPro_Discord_API {
	...
	*/
	...
	// 			error_log(print_r('Not $access_token', true));
			}
		}
	}
	...
	public function atm_saviorpro_discord_as_handler_add_member_to_guild( $_atm_saviorpro_discord_user_id, $user_id, $access_token ) {
		...
		if( !empty( $discord_assigned_member_role_ids ) && is_array( $discord_assigned_member_role_ids ) )
		{
			...
			if( in_array( 1051796132445044796, $discord_assigned_member_role_ids ) || in_array( 1014640051998691409, $discord_assigned_member_role_ids ))
			{
			...
			update_user_meta( $user_id, '_atm_saviorpro_discord_role_ids', $discord_roles );
			if( in_array( 1051796132445044796, $discord_roles ) || in_array( 1014640051998691409, $discord_roles ))
			{
				update_user_meta( $user_id, '_atm_saviorpro_discord_is_premium_plays', true );
			}
...
// 			}
	...
	public function atm_saviorpro_discord_assigned_member_role_ids( $user_id = false) {
		...
		if ( $guild_id && $discord_bot_token && !empty($_atm_saviorpro_discord_user_id) ) {
			error_log(print_r('$_atm_saviorpro_discord_user_id',true));
			error_log(print_r($_atm_saviorpro_discord_user_id,true));
...
// 			error_log( print_r('atm_saviorpro_discord_as_handler_put_memberrole else ====>', true));
		}
	}
