...
class SaviorPro_Discord_API {
	function __construct() {
		...
		// add_shortcode( 'test', array( $this, 'add_discord_role_manual' ));

	}
	/**
	 * Manual assign discord role by test page
	 * 
	 */
	public function add_discord_role_manual()
	{
		// atm_saviorpro_discord_assigned_member_role_ids
		$users = get_users( array( 'role__in' => [ 'subscriber' ], 'fields' => array( 'ID' ) ) );//, 'number' => 10
		$i=1;
		foreach($users as $user){
			if( 
				get_user_meta($user->ID, '_atm_saviorpro_discord_user_id', true) && 
				get_user_meta($user->ID, '_atm_saviorpro_discord_access_token', true) 
			){
				echo"<pre>";
			
				$memberships = wc_memberships_get_user_active_memberships( $user->ID ); // adding the args bit to get all memberships
				
				if ( ! empty( $memberships ) ) {
					// sleep(1);
					// $member_role_ids = $this->atm_saviorpro_discord_assigned_member_role_ids( $user->ID );
					...
					if( get_user_meta($user->ID, '_atm_saviorpro_discord_role_ids', true) )//1014640051998691409
					{
						// "1014640051998691409",
						// "927944057106366507"
						// $this->atm_saviorpro_discord_as_handler_put_memberrole( $user->ID, 1014640051998691409, false );
						$user_role_ids = empty(get_user_meta($user->ID, '_atm_saviorpro_discord_role_ids', true)) ? array(1014640051998691409) : get_user_meta($user->ID, '_atm_saviorpro_discord_role_ids', true);
						$user_role_ids = array_unique(array_filter(array_merge($user_role_ids, array(1014640051998691409))));
						// update_user_meta($user->ID, '_atm_saviorpro_discord_role_ids', $user_role_ids);
						print_r( get_user_meta($user->ID, '_atm_saviorpro_discord_role_ids', true) );
						// echo "Discord User role id updated : 927944057106366507<br>";
					}
					$i++;
				}
				// if( !empty($member_role_ids) )
				// {
				// 	echo"Not assigned Premium Plays";
				// }
				echo"</pre>";
			}
	...
	*/
	...
	public function atm_saviorpro_discord_as_handler_put_memberrole( $user_id, $role_id, $is_schedule ) {
		...
		if ( $access_token && $_atm_saviorpro_discord_user_id ) {
			...
			error_log(print_r('atm_saviorpro_discord_as_handler_put_memberrole',true));
